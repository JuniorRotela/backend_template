...
import getDataProduccionListViewDia from "../services/graficos/getProduccionListViewDia";
...
export const getReportesViewDia = async (req: Request, res: Response) => {
  const tableName = "reporteView"; // Reemplaza con el nombre de tu tabla
  const data = req.body;
   console.log("🚀 ~ getProduccionView ~ data:", data)
  ...
  // console.log("🚀 ~ getProduccionView ~ fecha:", fecha)
  try {
    const userData = await getDataProduccionListViewDia(fecha);
    res.json(userData);
  } catch (error) {
    console.error("Error getting product data:", error);
