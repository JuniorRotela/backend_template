...
import { insertData} from "../services/precios/Insert.services";
// import Garden from "../interface/garden";
import { getOneData } from "../genericQueries/getOne.services";
import { deleteGardenData } from "../services/delete.services";
import { getData } from "../genericQueries/getBuilder";
import { updateData } from "../services/precios/update.services";
// import Category from "../interface/category";
import Descuento from "../interface/descuento";
import Precios from "../interface/precios";


export const createPrecios = async (req: Request, res: Response) => {
  const tableName = "precios"; // Reemplaza con el nombre de tu tabla
  const data: Precios = req.body;
...
export const getPrecios = async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error("Error getting precios data:", error);
...
// tu controlador
export const updatePrecios = async (req: Request, res: Response) => {
  const tableName = "precios";
  const newData: Precios = req.body;
...
export const getOnePrecios = async (req: Request, res: Response) => {
  const { id } = req.params;
  const tableName = "precios"; // Reemplaza con el nombre de tu tabla
  ...
  try {
    ...
    if (Data) {
      ...
      res.status(404).json({ message: "precios not found" });
    }
  } catch (error) {
    console.error("Error getting precios data:", error);
