...
import { createData} from "../services/cliente/create.services";
...
import SalaLimpia from "../interface/salaLimpia";

import getsalabyFecha from "../services/salaLimpia/getAmbienteF";
import { insertDataSala } from "../services/salaLimpia/Insert.servicesS";
import { getUltimoRegistro } from "../services/salaLimpia/getUltimoRegistro.services";


export const createSala = async (req: Request, res: Response) => {
  const tableName = "sala_limpia"; // Reemplaza con el nombre de tu tabla
  const data: SalaLimpia = req.body;
  // console.log("datos recibidos controller",data);
  try {
    // Utiliza el servicio para insertar los datos
    const resp = await insertDataSala(tableName, data);
...
export const getSala = async (req: Request, res: Response) => {
  const tableName = "sala_limpia"; // Reemplaza con el nombre de tu tabla
  const data = req.body;
  // console.log("🚀 ~ getSala ~ data:", data)
  
  const fecha = data.fecha;
  // console.log("🚀 ~ getSala ~ fecha:", fecha)
  try {
    const gardenData = await getsalabyFecha(fecha);
    res.json(gardenData);
  } catch (error) {    
...
export const getRegistroFinal = async (req: Request, res: Response) => {
  const tableName = "sala_limpia"; // Reemplaza con el nombre de tu tabla

  try {
    const userData = await getUltimoRegistro(tableName);
    res.json(userData);
  } catch (error) {
    console.error("Error getting product data:", error);
...
export const getHistorialTemperaruta = async (req: Request, res: Response) => {
  const tableName = "infochapaview"; // Reemplaza con el nombre de tu tabla
