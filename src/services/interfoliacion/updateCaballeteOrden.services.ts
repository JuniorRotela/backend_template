...
import { generateUpdateCab } from './updateBuilderCab';

export const updateDataCabOrder= async (tableName: string, newData: any): Promise<void> => {
  try {
    ...
    // const updateQuery = await generateUpdateQuery(tableName, orden, data);
    const updateQuery = await generateUpdateCab(tableName, data);
