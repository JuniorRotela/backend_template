...
// const createMarcacionUrl = 'http://192.168.88.90:5003/marcacionEntrada';
...
// Buffer para almacenar eventos
const eventBuffer: any[] = [];

// Retraso entre eventos
const delay = 1500; // 3 segundos

// Enlace del controlador
const createMarcacionUrl = 'http://192.168.88.69:5003/marcacionEntrada';

// Middleware para recibir XML como texto
eventApp.use(express.text({ type: 'application/xml' }));

// Control para evitar múltiples ejecuciones simultáneas
let isProcessing = false;

// Mapa para controlar marcaciones por usuario
const lastMarkedTimes = new Map<string, number>();
const cooldown = 1 * 60 * 1000; // 2 minutos en milisegundos
...
const processEventsWithDelay = async () => {
    ...
    while (eventBuffer.length > 0) {
        ...
        if (accessEvent) {
            ...
            if (!employeeNoString) continue;

            const { dateTime } = event.AccessControllerEvent;

            const now = Date.now();
            const lastTime = lastMarkedTimes.get(employeeNoString);

            if (lastTime && now - lastTime < cooldown) {
                console.log(`⏱️ Usuario ${employeeNoString} ya marcó recientemente. Esperar 2 minutos.`);
                continue;
            }
            ...
            try {
                const response = await axios.post(createMarcacionUrl, datosUpdate);
                console.log('✅ Respuesta del controlador:', response.data);
                lastMarkedTimes.set(employeeNoString, now);
            } catch (error) {
                console.error('❌ Error al enviar datos al controlador:', error);
            }
        } else {
